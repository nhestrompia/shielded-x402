...
function deriveHttpServiceCandidates(baseUrl, serviceNames) {
  ...
  for (const service of serviceNames) {
    try {
      push(new URL(`/api/${service}`, baseUrl).toString());
      push(new URL(`/api/${service}/invoke`, baseUrl).toString());
    } catch {}
  }
  return candidates;
}
...
function deriveA2AInvokeCandidates(cardUrl, cardRaw) {
  ...
  if (
    ...
    for (const key of Object.keys(entrypoints)) {
      try {
        push(new URL(`/${key}/invoke`, base).toString());
      } catch {}
    }
  }
  ...
  if (Array.isArray(skills)) {
    for (const skill of skills) {
      ...
      if (!id) continue;
      try {
        push(new URL(`/${id}/invoke`, base).toString());
      } catch {}
    }
  }
...
if (providers.length === 0) {
  throw new Error(
    "Set at least one provider env (ERC8004_ENVIO_GRAPHQL_URL / ERC8004_SCAN_API_URL / onchain)",
  );
}
...
const rows = await directoryClient.search({
  chainId,
  isTestnet,
  limit,
  offset: 0,
...
console.log(`[scan] candidates=${ranked.length} (service-url discovery mode)`);
  ...
  for (const service of profile.services) {
    ...
    let candidates =
      service.protocol === "web"
        ? deriveHttpServiceCandidates(url, serviceNames)
        : [url];
...
console.log(
  "[found] none (no discovered endpoint returned a 402 x402 challenge)",
);
process.exit(1);
