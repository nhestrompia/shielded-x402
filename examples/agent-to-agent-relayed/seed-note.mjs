...
if (!rpcUrl)
  throw new Error("Set BASE_SEPOLIA_RPC_URL (or POOL_RPC_URL/SEPOLIA_RPC_URL)");
if (!usdcAddress || !isAddress(usdcAddress))
  throw new Error("Set valid USDC_ADDRESS");
...
if (!depositorPrivateKey || !depositorPrivateKey.startsWith("0x")) {
  throw new Error(
    "Set DEPOSITOR_PRIVATE_KEY (or DEPLOYER_PRIVATE_KEY/PAYER_PRIVATE_KEY)",
  );
}

const useFixedRho =
  (process.env.SEED_USE_FIXED_RHO ?? "false").trim().toLowerCase() === "true";
...
const useFixedNullifierSecret =
  (process.env.SEED_USE_FIXED_NULLIFIER_SECRET ?? "false")
    .trim()
    .toLowerCase() === "true";
const rawNullifierSecret = useFixedNullifierSecret
  ? process.env.NULLIFIER_SECRET?.trim()
  : undefined;
...
for (const log of receipt.logs) {
  if (
    !log.address ||
    log.address.toLowerCase() !== shieldedPoolAddress.toLowerCase()
  )
    continue;
  try {
...
console.log(
  `[seed-note] SEED_USE_FIXED_NULLIFIER_SECRET=${useFixedNullifierSecret}`,
);
