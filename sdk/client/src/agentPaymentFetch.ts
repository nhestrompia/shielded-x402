...
export type AgentPaymentFetch = (
  target: AgentTarget,
  init?: RequestInit,
) => Promise<Response>;

function endpointFromSelection(
  selection: CounterpartySelectionResult,
): string | undefined {
  return selection.selected?.endpoint.url;
}

function extractX402Payments(
  card: Record<string, unknown>,
): A2AX402PaymentProfile[] {
...
export function createAgentPaymentFetch(
  config: AgentPaymentFetchConfig,
): AgentPaymentFetch {
