...
function bytesHexToUtf8OrHex(value) {
    if (typeof value !== 'string')
        return undefined;
...
function readServices(registration) {
    const out = {};
    const setService = (protocol, value) => {
        if (!value)
            return;
        if (typeof value === 'string') {
            if (protocol === 'a2a')
                out.a2aEndpoint = value;
            if (protocol === 'mcp')
                out.mcpEndpoint = value;
            if (protocol === 'web')
                out.webEndpoint = value;
            if (protocol === 'oasf')
                out.oasfEndpoint = value;
            if (protocol === 'did')
                out.didIdentifier = value;
            if (protocol === 'ens')
                out.ensIdentifier = value;
            if (protocol === 'email')
                out.emailIdentifier = value;
            return;
        }
        if (typeof value !== 'object' || Array.isArray(value))
            return;
        const record = value;
        const endpoint = toNullableString(record.endpoint) ??
            toNullableString(record.url) ??
            toNullableString(record.identifier);
        if (!endpoint)
            return;
        if (protocol === 'a2a')
            out.a2aEndpoint = endpoint;
        if (protocol === 'mcp')
            out.mcpEndpoint = endpoint;
        if (protocol === 'web')
            out.webEndpoint = endpoint;
        if (protocol === 'oasf')
            out.oasfEndpoint = endpoint;
        if (protocol === 'did')
            out.didIdentifier = endpoint;
        if (protocol === 'ens')
            out.ensIdentifier = endpoint;
        if (protocol === 'email')
            out.emailIdentifier = endpoint;
    };
...
function createEmptyProfile(id, chainId, tokenId, blockNumber, blockTimestamp) {
    return {
        id,
        chainId,
        tokenId,
...
async function getOrCreateProfile(context, chainId, agentId, blockNumber, blockTimestamp) {
...
function applyRegistrationData(profile, registration) {
    const services = readServices(registration);
    const supportedTrust = Array.isArray(registration.supportedTrust)
        ? (registration.supportedTrust.filter((entry) => typeof entry === 'string').join(',') || undefined)
        : undefined;
    return {
        ...profile,
...
IdentityRegistry.Transfer.handler(async ({ event, context }) => {
    ...
    const profile = await getOrCreateProfile(context, event.chainId, event.params.tokenId, blockNumber, blockTimestamp);
...
IdentityRegistry.Registered.handler(async ({ event, context }) => {
    const blockNumber = toBigInt(event.block.number);
    const blockTimestamp = toBigInt(event.block.timestamp);
    const profile = await getOrCreateProfile(context, event.chainId, event.params.agentId, blockNumber, blockTimestamp);
...
IdentityRegistry.MetadataSet.handler(async ({ event, context }) => {
    ...
    const blockTimestamp = toBigInt(event.block.timestamp);
    const profile = await getOrCreateProfile(context, event.chainId, event.params.agentId, blockNumber, blockTimestamp);
    const metadataKey = toNullableString(event.params.metadataKey)?.toLowerCase() ??
    ...
    if (metadataKey && metadataValue) {
        if (metadataKey === 'agentwallet') {
            ...
            if (bool !== undefined)
                updated = { ...updated, x402Supported: bool };
        }
